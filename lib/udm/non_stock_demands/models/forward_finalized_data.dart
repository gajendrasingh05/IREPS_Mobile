class ForwardfinalizedData {
  String? apiFor;
  String? count;
  String? status;
  String? message;
  List<FwdfndData>? data;

  ForwardfinalizedData(
      {this.apiFor, this.count, this.status, this.message, this.data});

  ForwardfinalizedData.fromJson(Map<String, dynamic> json) {
    apiFor = json['api_for'];
    count = json['count'];
    status = json['status'];
    ...
    if (json['data'] != null) {
      data = <FwdfndData>[];
      json['data'].forEach((v) {
        data!.add(new FwdfndData.fromJson(v));
      });
    }
  }
...
class FwdfndData {
  ...
  FwdfndData.fromJson(Map<String, dynamic> json) {
    fileno = json['fileno'];
