...
class ForwardedfinalisedScreen extends StatefulWidget {
  const ForwardedfinalisedScreen({Key? key}) : super(key: key);

  @override
  State<ForwardedfinalisedScreen> createState() => _ForwardedfinalisedScreenState();
}

class _ForwardedfinalisedScreenState extends State<ForwardedfinalisedScreen> {
  ...
  void _onScrollEvent() {
    ...
    if(extentAfter == listScrollController.position.maxScrollExtent){
      Provider.of<ChangeScrollVisibilityProvider>(context, listen: false).setFwdVisibilityValue(false);
      Provider.of<ChangeScrollVisibilityProvider>(context, listen: false).setfwdScrollValue(true);
    }
  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) {
      ...
      Provider.of<NonStockDemandViewModel>(context, listen: false).getfwdfinalizedData("Forwarded_Finalized", fromdate, todate, context);
      Provider.of<ChangeScrollVisibilityProvider>(context, listen: false).setFwdVisibilityValue(false);
      Provider.of<ChangeScrollVisibilityProvider>(context, listen: false).setfwdScrollValue(false);
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
       floatingActionButton: Consumer<ChangeScrollVisibilityProvider>(builder: (context, value, child){
         if(value.getFwdVisibilityValue == true){
            return Container(
              height: 45,
              width: 45,
              ...
              child: FloatingActionButton(
                ...
                if(listScrollController.hasClients){
                  if(value.fwdUiscrollValue == false){
                    final position = listScrollController.position.maxScrollExtent;
                    listScrollController.jumpTo(position);
                    value.setfwdScrollValue(true);
                  }
                  else{
                    final position = listScrollController.position.minScrollExtent;
                    listScrollController.jumpTo(position);
                    value.setfwdScrollValue(false);
                  }
                }
              }, child: value.getFwdUiScrollValue == true ? Icon(Icons.arrow_upward, color: Colors.white) : Icon(Icons.arrow_downward_rounded, color: Colors.white)),
            );
         }
         else{
           return SizedBox();
           ...
           children: [
             ...
             Expanded(
                 child: Consumer<NonStockDemandViewModel>(builder: (context, value, child){
                    if(value.fwdfinalizedState == FwdFinalizedDataState.Busy){
                    ...
                    else if(value.fwdfinalizedState == FwdFinalizedDataState.Finished){
                       return ListView.builder (
                           itemCount: value.fwdfinalizeditems.length,
                           shrinkWrap: true,
                                                     ...
                                                     children: [
                                                       Column(
                                                         ...
                                                         children: [
                                                           ...
                                                           ConstrainedBox(
                                                             ...
                                                             child: value.fwdfinalizeditems[index].dmdno == null ? Text("NA", style: TextStyle(color: Colors.black, fontSize: 16)) : value.fwdfinalizeditems[index].demandref != null ? Wrap(
                                                               alignment: WrapAlignment.start,
                                                               children: <Widget>[
                                                                 Text("${value.fwdfinalizeditems[index].dmdno.toString()} ", style: TextStyle(color: Colors.black, fontSize: 16)),
                                                                 Text("Dt. ${value.fwdfinalizeditems[index].dmddate.toString().split(" ").first} ", style: TextStyle(fontSize: 16, color: Colors.black)),
                                                                 Text("${value.fwdfinalizeditems[index].demandref.toString()}", style: TextStyle(fontSize: 16, color: Colors.indigo.shade500)),
                                                           ...
                                                           // ConstrainedBox(
                                                           //   constraints: BoxConstraints(maxWidth: size.width * 0.9),
                                                           //   child: value.fwdfinalizeditems[index].dmdno == null ? Text("NA", style: TextStyle(color: Colors.black, fontSize: 16)) : Wrap(
                                                           //     alignment: WrapAlignment.start,
                                                           //     children: <Widget>[
                                                           //       Text("${value.fwdfinalizeditems[index].dmdno.toString()} ", style: TextStyle(color: Colors.black, fontSize: 16)),
                                                           //       Text("Dt. ${value.fwdfinalizeditems[index].dmddate.toString().split(" ").first} ", style: TextStyle(fontSize: 16, color: Colors.black)),
                                                       ...
                                                       Column(
                                                         ...
                                                         children: [
                                                           ...
                                                           ConstrainedBox(
                                                             constraints: BoxConstraints(maxWidth: size.width * 0.9),
                                                             child: value.fwdfinalizeditems[index].indentorname == null ? Text("NA", style: TextStyle(color: Colors.black, fontSize: 16)) :  Wrap(
                                                               alignment: WrapAlignment.start,
                                                               children: <Widget>[
                                                                 Text("${value.fwdfinalizeditems[index].indentorname.toString().split("<br/>").first} ", style: TextStyle(color: Colors.black, fontSize: 16)),
                                                                 Text("${value.fwdfinalizeditems[index].indentorname.toString().split("<br/>").last}", style: TextStyle(fontSize: 16, color: Colors.black)),
                                                                 //getCoveredDuePoDetail(value.coveredDueData[index].podt.toString(), value.coveredDueData[index].firm.toString(), value.coveredDueData[index].pocat.toString().trim())
                                                       ...
                                                       Column(
                                                         ...
                                                         children: [
                                                           ...
                                                           ConstrainedBox(
                                                             constraints: BoxConstraints(maxWidth: size.width * 0.9),
                                                             child: value.fwdfinalizeditems[index].itemdescription == null ? Text("NA", style: TextStyle(color: Colors.black, fontSize: 16)) :  Wrap(
                                                               alignment: WrapAlignment.start,
                                                               children: <Widget>[
                                                                 Text(parse(parse(value.fwdfinalizeditems[index].itemdescription.toString()).body!.text).documentElement!.text, style: TextStyle(color: Colors.black, fontSize: 16)),
                                                                 //Text("${value.fwdfinalizeditems[index].itemdescription.toString()}", style: TextStyle(color: Colors.black, fontSize: 16)),
                                                               ],
                                                             ),
                                                           ),
                                                           //Text("P.O.No. ${value.coveredDueData[index].poNo.toString()} dt. ${value.coveredDueData[index].podt.toString()} on M/s. ${value.coveredDueData[index].firm.toString()} PO-CAT:[ XX ]", style: TextStyle(color: Colors.black, fontSize: 16))
                                                       ...
                                                       Column(
                                                         ...
                                                         children: [
                                                           ...
                                                           ConstrainedBox(
                                                             constraints: BoxConstraints(maxWidth: size.width * 0.9),
                                                             child: value.fwdfinalizeditems[index].demandestval == null ? Text("NA", style: TextStyle(color: Colors.black, fontSize: 16)) : Wrap(
                                                               alignment: WrapAlignment.start,
                                                               children: <Widget>[
                                                                 Text("Rs. ${value.fwdfinalizeditems[index].demandestval.toString()}/-", style: TextStyle(color: Colors.black, fontSize: 16)),
                                                                 Text(" ${value.fwdfinalizeditems[index].approvalvalue.toString()}", style: TextStyle(fontSize: 16, color: Colors.green.shade500)),
                                                                 //getCoveredDuePoDetail(value.coveredDueData[index].podt.toString(), value.coveredDueData[index].firm.toString(), value.coveredDueData[index].pocat.toString().trim())
                                                       ...
                                                       Column(
                                                         ...
                                                         children: [
                                                           ...
                                                           ConstrainedBox(
                                                             constraints: BoxConstraints(maxWidth: size.width * 0.9),
                                                             child: value.fwdfinalizeditems[index].username == null ? Text("NA", style: TextStyle(color: Colors.black, fontSize: 16)) : Wrap(
                                                               alignment: WrapAlignment.start,
                                                               children: <Widget>[
                                                                 Text("${value.fwdfinalizeditems[index].username.toString()} ", style: TextStyle(color: Colors.black, fontSize: 16)),
                                                                 Text("${value.fwdfinalizeditems[index].currentlywith.toString()} ", style: TextStyle(color: Colors.black, fontSize: 16)),
                                                                 Text("${value.fwdfinalizeditems[index].useremail.toString()}", style: TextStyle(fontSize: 16, color: Colors.blue.shade200, decoration: TextDecoration.underline)),
                                                                 //getCoveredDuePoDetail(value.coveredDueData[index].podt.toString(), value.coveredDueData[index].firm.toString(), value.coveredDueData[index].pocat.toString().trim())
                                                         ...
                                                         children: [
                                                           ...
                                                           ConstrainedBox(
                                                             ...
                                                             child: Wrap(
                                                               alignment: WrapAlignment.start,
                                                               children: <Widget>[
                                                                 Text("${value.fwdfinalizeditems[index].dmdstatusval.toString()} ", style: TextStyle(color: Colors.black, fontSize: 16)),
                    ...
                    else if(value.fwdfinalizedState == FwdFinalizedDataState.NoData){
                      return Center(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.center,
                          mainAxisAlignment: MainAxisAlignment.center,
